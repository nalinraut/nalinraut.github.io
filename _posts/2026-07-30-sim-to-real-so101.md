...
<p style="font-size: 0.85em; color: #666; margin-top: 0.3em;">Loss drops rapidly in the first 2K steps, plateaus through 10K, then resumes with a lower learning rate for the final 10K steps.</p>
</div>
...
<p style="font-size: 0.85em; color: #666; margin-top: 0.3em;">GPU utilization averaged 92%, peaking at 98%. VRAM held steady at 33.5 GB / 34.1 GB, fully saturated.</p>
</div>

We needed `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True` to avoid OOM from CUDA memory fragmentation during checkpoint resume.

### Real robot evaluation

We evaluate with [FRAME](https://github.com/AmeyaWagh/robometric-frame) robotics metrics, logged to MLflow. The key diagnostic turned out to be per-joint MSE decomposition:

| Checkpoint | Success | Action MSE | Gripper MSE | Notes |
|------------|---------|------------|-------------|-------|
| `finetune_cotrain/checkpoint-20000` (Jul 4) | 1/1 | 84.6 | 69.5 | Gripper actively commanded |
| `so101_finetuned/checkpoint-20000` (Jul 19) | 0/1 | 6.7 | 0.2 | Gripper nearly static |

The failed checkpoint had *lower* overall MSE (6.7 vs 84.6), which is counterintuitive until you look at the gripper channel. A gripper MSE of 0.2 means the policy barely moves the gripper. It tries to pick without opening the jaw. The working checkpoint has gripper MSE of 69.5 because it is aggressively opening and closing, which creates large instantaneous errors but achieves the task. Low MSE does not mean good policy; per-joint decomposition tells the real story.

#### Working checkpoint (Jul 4, `finetune_cotrain/checkpoint-20000`)

<div style="text-align: center; margin: 1.5em 0;">
<img src="/assets/img/skillgen/eval_success_trajectory.png" alt="Per-joint trajectory comparison for successful eval" style="width: 100%; max-width: 800px;">
<p style="font-size: 0.85em; color: #666; margin-top: 0.3em;">Per-joint trajectory: commanded (blue) vs executed (orange). Note the gripper channel with large excursions for open/close.</p>
</div>

<div style="display: flex; gap: 1em; flex-wrap: wrap; justify-content: center; margin: 1.5em 0;">
<div style="flex: 1; min-width: 300px; max-width: 400px;">
<img src="/assets/img/skillgen/eval_success_metrics.png" alt="FRAME metrics summary for successful eval" style="width: 100%;">
<p style="font-size: 0.85em; color: #666; text-align: center;">FRAME metrics summary, success=1.0</p>
</div>
<div style="flex: 1; min-width: 300px; max-width: 400px;">
<img src="/assets/img/skillgen/eval_success_tracking.png" alt="Tracking error over time for successful eval" style="width: 100%;">
<p style="font-size: 0.85em; color: #666; text-align: center;">Tracking error over time, spikes at grasp/place transitions</p>
</div>
</div>

#### Failed checkpoint (Jul 19, `so101_finetuned/checkpoint-20000`)

<div style="text-align: center; margin: 1.5em 0;">
<img src="/assets/img/skillgen/eval_failed_trajectory.png" alt="Per-joint trajectory comparison for failed eval" style="width: 100%; max-width: 800px;">
<p style="font-size: 0.85em; color: #666; margin-top: 0.3em;">Per-joint trajectory: the gripper channel is nearly flat. The policy is not commanding open/close.</p>
</div>

<div style="display: flex; gap: 1em; flex-wrap: wrap; justify-content: center; margin: 1.5em 0;">
<div style="flex: 1; min-width: 300px; max-width: 400px;">
<img src="/assets/img/skillgen/eval_failed_metrics.png" alt="FRAME metrics summary for failed eval" style="width: 100%;">
<p style="font-size: 0.85em; color: #666; text-align: center;">FRAME metrics: lower MSE but success=0.0</p>
</div>
<div style="flex: 1; min-width: 300px; max-width: 400px;">
<img src="/assets/img/skillgen/eval_failed_tracking.png" alt="Tracking error over time for failed eval" style="width: 100%;">
<p style="font-size: 0.85em; color: #666; text-align: center;">Tracking error: smoother (less gripper motion) but the task fails</p>
</div>
</div>
